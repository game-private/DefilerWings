...
REGEXP_FILTER_BUTTONS = re.compile('img/sex_screen/elf/red/3/button_.*_idle.png')
...
# Новая функция для создания скринов девушек
def load_focus_mask_girls(coordinate_path):    
    coordinates_dict = {}

    coordinates_file = renpy.file(coordinate_path)
    coordinates_buffer = coordinates_file.read()
    coordinates_file.close()
    ...
    FocusMaskGirls.coordinates_dict = coordinates_dict 

# Новая функция для создания скринов драконов
def load_focus_mask_dragons(coordinate_path):    
    coordinates_dict = {}
    ...
    FocusMaskDragons.coordinates_dict = coordinates_dict 

# Новая функция для создания скринов Архимонда
# Элуна, создавать 100500 одинаковых функций - это ужасно!
def load_focus_mask_archimonde(coordinate_path):    
    coordinates_dict = {}
    ...
    FocusMaskArchimonde.coordinates_dict = coordinates_dict 

class FocusMaskGirls:
    
    def __init__(self, target,relative_path):
        self.button_image_path = relative_path + "/button_%s_idle.png" % target
        
    def __call__(self, x, y):
        coordinates = FocusMaskGirls.coordinates_dict[self.button_image_path]
        return (x // RESOLUTION_DIVIDER, y // RESOLUTION_DIVIDER) in coordinates

class FocusMaskDragons:
    
    def __init__(self, target,relative_path):
        self.button_image_path = relative_path + "/button_%s_idle.png" % target
        
    def __call__(self, x, y):
        coordinates = FocusMaskDragons.coordinates_dict[self.button_image_path]
        return (x // RESOLUTION_DIVIDER, y // RESOLUTION_DIVIDER) in coordinates

class FocusMaskArchimonde:
    
    def __init__(self, target,relative_path):
        self.button_image_path = relative_path + "/button_%s_idle.png" % target
        
    def __call__(self, x, y):
        coordinates = FocusMaskArchimonde.coordinates_dict[self.button_image_path]
        return (x // RESOLUTION_DIVIDER, y // RESOLUTION_DIVIDER) in coordinates