...
#       Задаём модификаторы
        if 'fire_breath' in self.game.dragon.modifiers() and not self.game.girl.type == 'fire':
          self.fire=True   # Огонь (кроме головы)
        if 'ice_breath' in self.game.dragon.modifiers() and not self.game.girl.type == 'ice':
          self.ice=True    # Лёд (кроме головы)
        if ('lightning_breath' in self.game.dragon.modifiers() or 'silver_magic' in self.game.dragon.modifiers()) and not self.game.girl.type == 'titan':
          self.lightning=True # Электричество (кроме головы)
        if 'poison_breath' in self.game.dragon.modifiers():
          self.poison=True   # Ядовитое дыхание  (кроме головы)
        if 'sound_breath' in self.game.dragon.modifiers():
          self.sound=True   # Звук (голова)
        if 'fear_of_dark' in self.game.dragon.modifiers() or 'aura_of_horror' in self.game.dragon.modifiers() or 'ugly' in self.game.dragon.modifiers():
