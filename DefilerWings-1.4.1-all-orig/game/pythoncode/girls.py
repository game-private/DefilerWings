...
# Рыцарь выпускает девушек на свободу
    ...
    def chromi_free_all_girls(self): # Хроми освобождаетпленниц.
        for girl_i in reversed(xrange(self.prisoners_count)):
            self.game.girl = self.prisoners[girl_i]
            if self.game.girl.love is None:
    ...
    def pythoness_free_all_girls(self): # Хроми освобождаетпленниц.
        for girl_i in reversed(xrange(self.prisoners_count)):
            self.game.girl = self.prisoners[girl_i]
            if self.game.girl.love is None:
...
# Все остальные типы логов.
    ...
    def escape_decision(self,girl_i):
                if self.game.girl.virgin == True:
                    ...
                    self.game.girl.third(text)
                if (random.randint(1, 3) == 1):   # Всё же решились на бегство
                  if self.game.girl.virgin == True:
...
#        self.spawn_list_girls.append()
    ...
    def girl_in_lair(self,girl_i):
        ...
        if self.game.girls_list.prisoners[girl_i].blind:
          ...
          add = u""
        d = u"%s, %s, %s, %s%s%s" %(self.game.girls_list.prisoners[girl_i].name, girls_data.girls_info[self.game.girls_list.prisoners[girl_i].type]['description'], girls_data.nature_info[self.game.girls_list.prisoners[girl_i].nature], status, love,add)
        return d
