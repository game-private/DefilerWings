...
transform bottom_to_top:  # Трансформ прокручивающий текст
        ...
        linear 200.0*mod ypos 0 yanchor 1.0  # За 200 секунд поднимаем текст до верхней кромки и смещаем якорь (по которому высчитывается положение текста)

transform intro_bg:
    "intro 1" with dissolve  # Начальная картинка (нужна чтобы показать ее если пошел повтор
    pause 37.5*mod  # Задержка перед следущей картинкой
...
screen sc_intro:
    ...
    timer 200.0*mod action Return()  # Едем дальше если слишком долго ждали