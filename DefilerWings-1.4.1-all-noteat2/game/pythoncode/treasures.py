...
material_description_rus = {
    ...
    "ivory": {
        'nominative': u'мумачья кость',
        'genitive': u'мумачьей кости'
    },
    "agate": {
        'nominative': u'агат',
        'genitive': u'агата'
...
gem_description_rus = {
    ...
    "elven_beryll": {
        'he': {
            'nominative': u'альвийский берилл',
            'genitive': u"альвийских берилла",
            'ablative': u'альвийским бериллом'
        },
        'they': {
            'genitive': u"альвийских бериллов",
            'ablative': u'альвийскими бериллами'
        }
    },
    "topaz": {
        'he': {
...
treasure_types = {  # допилить типы сокровищ
    "dish": (5, "it", True, False, False, False, True),     # Блюдо
    "goblet": (4, "he", True, False, False, True, True),    # Кубок
    "cup": (3, "she", False, True, False, False, True),     # Чаша
    "casket": (5, "she", True, True, False, True, True),    # Шкатулка
    "statue": (10, "she", True, True, True, False, False),  # Статуэтка
    "tabernacle": (5, "she", True, True, False, True, True),# Дарохранительница
    "icon": (10, "she", True, False, True, False, False),   # Икона
    "tome": (10, "he", True, False, False, True, True),     # Фолиант
    "comb": (2, "he", True, True, False, False, True),      # Гребень
    "phallos": (3, "he", True, True, False, False, True),   # Фаллос
    "mirror": (4, "it", True, True, False, True, True),     # Зеркальце
    "band": (3, "he", True, False, False, False, False),    # Обруч
    "diadem": (10, "she", True, False, False, True, False), # Диадема
    "tiara": (15, "she", True, False, False, True, False),  # Тиара
    "earring": (1, "she", True, False, False, True, False), # Серьга
    "necklace": (5, "it", True, False, False, True, False), # Ожерелье
    "pendant": (3, "he", True, False, False, False, True),  # Кулон
    "ring": (2, "it", True, True, False, False, False),     # Колечко
    "broch": (3, "she", True, False, False, True, False),   # Брошь
    "gemring": (5, "he", True, True, False, True, False),   # Перстень
    "seal": (3, "he", True, True, False, False, True),      # Перстень-печатка
    "armbrace": (3, "he", True, True, False, True, True),   # Браслет
    "legbrace": (4, "he", True, True, False, True, True),   # Ножной браслет
    "crown": (15, "she", True, False, False, True, False),  # Корона
    "scepter": (15, "he", True, False, False, True, False), # Скипетр
    "chain": (3, "she", True, False, False, False, False),  # Цепь 
    "fibula": (2, "she", True, False, False, False, True),  # Фибула
}
...
treasure_description_rus = {  # допилить типы сокровищ
    "dish": {'nominative': u'блюдо', 'ablative': u'блюде', 'pluralia': u'Блюда' },
    "goblet": {'nominative': u'кубок', 'ablative': u'кубке', 'pluralia': u'Кубки'},
    "cup": {'nominative': u'чаша', 'ablative': u'чаше', 'pluralia': u'Чаши'},
    "casket": {'nominative': u'шкатулка', 'ablative': u'шкатулке', 'pluralia': u'Шкатулки'},
    "statue": {'nominative': u'статуэтка', 'ablative': u'статуэтке', 'pluralia': u'Статуэтки'},
    "tabernacle": {'nominative': u'дарохранительница', 'ablative': u'дарохранительнице', 'pluralia': u'Дарохранительницы'},
    "icon": {'nominative': u'икона', 'ablative': u'иконе', 'pluralia': u'Иконы'},
    "tome": {'nominative': u'фолиант', 'ablative': u'фолианте', 'pluralia': u'Фолианты'},
    "comb": {'nominative': u'гребень', 'ablative': u'гребене', 'pluralia': u'Гребни'},
    "phallos": {'nominative': u'фаллос', 'ablative': u'фаллосе', 'pluralia': u'Фаллосы'},
    "mirror": {'nominative': u'зеркальце', 'ablative': u'зеркальце', 'pluralia': u'Зеркальца'},
    "band": {'nominative': u'обруч', 'ablative': u'обруче', 'pluralia': u'Обручи'},
    "diadem": {'nominative': u'диадема', 'ablative': u'диадеме', 'pluralia': u'Диадемы'},
    "tiara": {'nominative': u'тиара', 'ablative': u'тиаре', 'pluralia': u'Тиары'},
    "earring": {'nominative': u'серьга', 'ablative': u'серьге', 'pluralia': u'Серьги'},
    "necklace": {'nominative': u'ожерелье', 'ablative': u'ожерелье', 'pluralia': u'Ожерелья'},
    "pendant": {'nominative': u'кулон', 'ablative': u'кулоне', 'pluralia': u'Кулоны'},
    "ring": {'nominative': u'колечко', 'ablative': u'колечке', 'pluralia': u'Колечки'},
    "broch": {'nominative': u'брошь', 'ablative': u'броше', 'pluralia': u'Броши'},
    "gemring": {'nominative': u'перстень', 'ablative': u'перстне', 'pluralia': u'Перстни'},
    "seal": {'nominative': u'перстень-печатка', 'ablative': u'перстне-печатке', 'pluralia': u'Перстни-печатки'},
    "armbrace": {'nominative': u'браслет', 'ablative': u'браслете', 'pluralia': u'Браслеты'},
    "legbrace": {'nominative': u'ножной браслет', 'ablative': u'браслете', 'pluralia': u'Ножные браслеты'},
    "crown": {'nominative': u'корона', 'ablative': u'короне', 'pluralia': u'Короны'},
    "scepter": {'nominative': u'скипетр', 'ablative': u'скипетре', 'pluralia': u'Скипетры'},
    "chain": {'nominative': u'цепь', 'ablative': u'цепи', 'pluralia': u'Цепи'},
    "fibula": {'nominative': u'фибула', 'ablative': u'фибуле', 'pluralia': u'Фибулы'},
}
...
image_description_rus = {
    ...
    'dragon_entwine_naked_girl': {
        ...
        'accusative': u'дракона, обвивающего обнаженную девушку',
    },
...
class Treasure(object):  # класс для сокровищ
    ...
    def __init__(self, treasure_type, alignment,girl_id=None):
    ...
    def craft_cost(self, base_cost, price_multiplier, by_dragon=False):  
        """
    ...
    def description(self, language='rus'):
        if language == 'rus':
            if self.quality == 'dragon':
                try:
                    quality_str = ' (' + str(math.floor(self.quality_mod*10)/10) + ') '
                except:
                    quality_str = ''
            else:
...
def gen_treas(count, t_list, alignment, min_cost, max_cost, obtained,girl_id=None):
    ...
    while count > 0:
        ...
        if treas_holder in gem_types:
            ...
            t = Treasure(treas_holder, alignment,girl_id=girl_id)
            t.obtained = obtained
...
#    @property
    def definitly_jewelry(self,tresure_i):
        if len(self.jewelry):
#            random_jewelry = random.choice(self.jewelry)
            return u"%s.\nСтоимость украшения: %s.\n%s" % (capitalize_first(tresure_i.description()),                                                           number_conjugation_rus(tresure_i.cost, u"фартинг"),                                                           tresure_i.obtained)
        else:
            return u"Украшений в сокровищнице нет"
...
#                    option_name = u"Из %s  (ценность: %s)" % (metal_description_rus[material_type]['genitive'],metal_types[material_type])
    ...
    def craft(self, is_crafting=False, quality=['random'], alignment=['random'], base_cost=0, price_multiplier=100, by_dragon=False):
        ...
        while menu_choice is not 'create':
            ...
            if is_crafting:
                if item.craft_cost(base_cost, price_multiplier, by_dragon) > 0:
                    price_msg = number_conjugation_rus(item.craft_cost(base_cost, price_multiplier), u"фартинг")
                    craft_msg = u"Смастерить за %s (есть %s, стоимость %s)" % (price_msg, self.money, item.cost)
                else:
                    craft_msg = u"Смастерить (стоимость %s)" %(item.cost)
            else:
                price_msg = number_conjugation_rus(item.craft_cost(base_cost, price_multiplier), u"фартинг")
...
#        self.active_treasure_botton = treasury_i
