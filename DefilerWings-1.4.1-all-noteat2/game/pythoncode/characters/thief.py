...
class Thief(Talker, Mortal):
    ...
    def __init__(self, level=1, treasury=None, *args, **kwargs):
        ...
        # @fdsc
        # Убрал ограничение уровня
        levelByMax = level
        if level < 1:
            level = 1
            levelByMax = 1
        elif level > Thief.max_level():
            levelByMax = Thief.max_level()

        self._skill = levelByMax
        self._title = data.thief_titles[levelByMax - 1]
...
#            thief.event("start_trap", trap=upgrade)
            ...
            # @fdsc Апгрейды логова могут складываться
            # 1/3 что вора ловушка заденет
            # for i in range(data.lair_upgrades[upgrade].protection):
            for i in range(int(math.floor(self._gameRef.lair.upgrades[upgrade].protection))):
...
#        thief(u"%s,%s" %(len(stolen_items), thief.skill))
        ...
        if 'gold_scale' in dragon.modifiers():
            por -= 1
            # Тени видят вора, который слишком сильно привлечён драконей чешуёй
            if 'shadow' in dragon.heads:
                por -= 2
