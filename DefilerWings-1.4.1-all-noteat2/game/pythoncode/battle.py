...
def battle_action(dragon, foe):
    ...
    if dragon_hit > foe_defence:
        achieve_target(foe.kind, "kill")    # событие для ачивок
#        narrator(u"kill, foe.name")
        foe.die()
...
def brute_chance(balance, attack, defence):
    ...
    # fdsc При слишком большой прокачке идёт переполнение
    # summ = float(sum(defence_list))
    summ = decimal.Decimal(sum(defence_list))
    defence_list = [float(a / summ) for a in defence_list]
    # расчет шанса
    victory = 0.0
