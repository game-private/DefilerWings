...
label lb_location_road_main:
    ...
    $ choices = [
        ...
        ("lb_castle_found", 5),
        ("lb_palace_found", 3),
        ("lb_summon_road", 6 * game.desperate),
...
label lb_summon_road:  # Призыв на равнинах
    if game.summon.seal>data.max_summon:
      jump lb_location_road_main   # При уже вызванном демоне встречи не происходит.
    else:
      'Неподалёку от обочины заброшенного тракта происходит кое-что интересное...'
      call lb_summon from _call_lb_summon_4
    return    
...
label lb_enc_tornament:
    ...
    menu:
        'Вызвать победителя на бой':
            ...
            $ text = u'%s с замиранием сердца гадала, выберут ли её по итогам турнира "королевой любви и красоты". Выбрали. Дракон выбрал.  \n\n' % game.girl.name
        ...
        'Не ввязываться' if game.dragon.bloodiness < 5:
            ...
            'Осторожность не повредит. Если этот рыцарь действительно лучший в округе, он может быть опасен. А девицу и кусок золота можно найти где-нибудь ещё...'
    return
...
label lb_enc_inn:
    ...
    menu:
        'Дыхнуть огнём' if doit:
            $ game.dragon.drain_energy()
            "Используя своё огненное дыхание, [game.dragon.name] поджигает здание с четырёх разных сторон. Деревянный трактир сгорает стремительно, погребая под пылающими обломками людей, забаррикадировавшихся внутри."
            $ game.poverty.value += 1
            $ game.dragon.reputation.points += 5
            '[game.dragon.reputation.gain_description]'
        'Наколдовать ядовитый туман' if game.dragon.mana > 0:    # @fdsc было magic
...
label lb_enc_peasant_cart:
    'По дороге медленно едет крестьянская телега, гружёная сеном. Такие повозки тут встречаются часто, и все как одна совершенно бесполезны. Даже лошадь на вид такая жилистая и заморенная, что не вызываетни особого аппетита. Аж злость берёт.'
    menu:
...
label lb_enc_carriage:
    'На дороге пыль стоит столбом, это едет карета благородной дамы с тяжеловооруженными конными арбалетчиками в качестве охраны. Добрая добыча, хотя и не самая простая...'
    ...
    menu:
        'Атаковать кортеж':
            ...
            'Теперь, когда охрана не представляет угрозы, можно заглянуть внутрь кареты. Разодрав её кузов, словно шуршащую подарочную упаковку, [game.dragon.name] обнаруживает внутри трёх женщин - очевидно мать, дочь и служанку. Старухи не представляют никакого интереса, а вот с девицей можно отлично развлечься!'
            $ game.dragon.reputation.points += 5
            '[game.dragon.reputation.gain_description]'
            $ description = game.girls_list.new_girl('princess')
            $ text = u'%s ехала к своему отцу, когда её жизнь полетела в тартарары. Дракон перебил кортеж, разодрал на части мать и наставницу. Сама девушка оказалась в полной власти кровожадной и похотливой ящерицы...  \n\n' % game.girl.name
            $ game.chronik.write_chronik(text,game.dragon.level,game.chronik.girl_id)
            nvl clear
            game.girl.third "[description]"
    ...
    menu:
        'Принять вызов':
            ...
            python:
                ...
                min_cost = 10
                max_cost = 250
                obtained = "Этот предмет принадлежал когда-то беззвестному странствующему рыцарю."
...
label lb_enc_trader:
    'На дороге показывается большой крытый фургон, расписанный красочными рекламными надписями. Это какой-то странствующий торговец, не слишком преуспевающий, но, судя по запаху, кое-какое серебро у него в карманах водится. Надо бы облегчить его ношу.'
    menu:
        'Вымогать деньги':
            python:
                game.dragon.drain_energy()
    ...
    menu:
        'Вымогать деньги' if game.dragon.fear > 3:
            python:
                ...
                game.lair.treasury.receive_treasures([gold_trs])
            'Караванщик с ворчанием отдаёт дракону несколько золотых дублонов, чтобы тот не трогал повозки и пропустил их дальше.'
            $ game.dragon.reputation.points += 1
            '[game.dragon.reputation.gain_description]'
        'Разграбить корован':
            $ game.dragon.drain_energy()
...
label lb_enc_lcaravan:
    '[game.dragon.name] решает подойти к вопросу обстоятельно и залегает в засаду в укрытой кустами придорожной канаве. Полдня ожидания наконец-то приносят достойную награду - на дороге появляется богатый купеческий караван. Судя по качеству и количеству охраны, эти торговцы платят золотом.'
    $ game.foe = Enemy('mounted_guard', game_ref=game)
    $ chances = show_chances(game.foe)
    menu:
        ...
        'Разграбить корован':
            ...
            'Перебив охрану и караванщиков, [game.dragon.name] отыскивает в разбитых телегах всё ценное. В основном тут разные ненужные уважающему себя дракону товары - ткани, специи, оливковое масло и тому подобное, но у купцов и наемников есть в кошельках звонкие монеты:'
            python:
                count = random.randint(5, 15)
                alignment = 'human'
                min_cost = 1
...
label lb_patrool_road:
    python:
        ...
        if chance < 4:
            ...
            dtxt = 'Пронзительный клич раздаётся с небес - это всадник на грифоне пикирует с высоты, завидев у дороги блеск драконьей чешуи.'
        else:
            patrool = 'angel'
